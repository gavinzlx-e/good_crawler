...
public class WebCrawler implements Runnable {
	...
	//  用PageFetcher.fetch爬取网页，如果curURL有redirect，则将redirect url的url加入Frontier，以后再调度；如果爬取正常
	//  ，则先进行parse，生成Page，将新urls降入Frontier(新加入url的深度此时确定)，调用visit(Page){用户自定义操作
			...
			if (statusCode < 200 || statusCode > 299) { // Not 2XX: 2XX status codes indicate success
				...
				for (WebURL webURL : parseData.getOutgoingUrls()) {
					...
					if (newdocid > 0) {
						...
						webURL.setDocid(newdocid);//已经处理的url
					} else {
						webURL.setDocid(-1); // 新的url，没有被处理的url
						webURL.setDepth((short) (curURL.getDepth() + 1));
						//小于最大dept
						if ((maxCrawlDepth == -1) || (curURL.getDepth() < maxCrawlDepth)) {
							//调用我们的自定义函数，判断是否应该处理这个新url
							if (shouldVisit(page, webURL)) {
								if (robotstxtServer.allows(webURL)) {
